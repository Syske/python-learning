...
# url = f'http://367hsck.cc/'
    url = f'https://wvamffg.info/suoyoushipin/guochan/index_{pageNum}.html'
    headers = {
    ...
    contents = selector1.css('.items > figure')
    # print(contents)
    index = 1
    datas = []
    ...
    for content in contents:
        ...
        # if index > 2:
            # print(content.get())
        href = url + content.css('.pos > a::attr(href)').get()
        print(href)
        img_url = content.css('.pos > a > img::attr(src)').get()
        print(img_url)
        title = content.css('.pos > a::attr(title)').get()
        print(title)
        if title != None:
